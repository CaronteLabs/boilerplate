	...
	before(function() {
		describe("Pre Test", function() {
			describe("Clear Cache", function() {
				it("should delete a just created json file", function() {
					var fs = require('fs'), u = '___a.json';
					//initialitze cache;
					var jsc = new jscache({name: u});
					//create a cache file;
					fs.writeFileSync(u, {});
					//delete cache 
					jsc.deleteCache(u.replace('.json', ''));
					//verify the file doesn't exist anymore
					assert.ok(!fs.existsSync(u));
					return;
				});
			});
		});

		new jscache().deleteCache('_jscache');
