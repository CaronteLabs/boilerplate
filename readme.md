...
You can managing your external assets the way you prefer, however a convenient utility is included in thi boilerplate, and it helps you managin assets more easily. 

##assets.js

Inside `src` folder a file called `assets.js` is included. A sample `assets.js` file is this:
```module.exports = {
  'index.html,post.html': {
    css: [
      {name: 'normalize', path: './libs/normalize.css/normalize.css'},
      {name: 'main', path: './css/main.css'}
    ],
    js: {
      pre: [
        {name: 'jquery', path: './libs/jquery/dist/jquery.js'},
        {name: 'main', path: './js/main.js'},
        {name: 'plugin', path: './js/plugins.js'}
      ],
      post: [
        {name: 'jquery', path: './libs/jquery/dist/jquery.js'},
        {name: 'main', path: './js/main.js'},
  ...
  'article.html': {
    css: [
      {name: 'normalize', path: './libs/normalize.css/normalize.css'},
      {name: 'main', path: './css/post.css'}
    ],
    js: [name: 'plugin', path: './js/plugins.js'}]
  }
}
```

The JSON-Schema for representing this data is the following: 
```
{
  "$schema": "schemauri.html",
  "type": "object",
  "patternProperties": {
    "*.html": {
      "type": object,
